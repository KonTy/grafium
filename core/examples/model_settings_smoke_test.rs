...
fn main() {
    ...
    let downloaded_model = PathBuf::from(
        args.next()
            .expect("usage: <downloaded-model> <data-dir> <source>"),
    );
    ...
    println!(
        "Settings: whisper.model = {:?} (nothing set)",
        config.whisper.model_ref.model
    );

    let transcriber =
        WhisperTranscriber::from_config(&config, &data_dir).expect("model resolution/load failed");
