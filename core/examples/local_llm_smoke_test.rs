...
async fn main() {
    ...
    let downloaded_model =
        PathBuf::from(args.next().expect("usage: <downloaded-gguf> <data-dir> <prompt>"));
    let data_dir = PathBuf::from(args.next().expect("usage: <downloaded-gguf> <data-dir> <prompt>"));
    let prompt = args.next().unwrap_or_else(|| "What is 2 + 2? Answer in one short sentence.".to_string());
    ...
    println!("Resolved + loaded model successfully from settings alone: {}", llm.name());
    ...
    let messages = vec![ChatMessage { role: MessageRole::User, content: prompt.clone() }];
    ...
    println!("\nPrompt: {prompt}\n--- response ---");
    let response = llm.complete(&messages, &options).await.expect("completion failed");
    println!("{response}");
}
