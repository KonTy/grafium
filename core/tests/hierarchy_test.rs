...
fn test_hierarchy_parent_child_creation() {
    ...
    assert!(titles.contains(&"test"),      "parent 'test' must be auto-created");
    assert!(titles.contains(&"test/page"), "child 'test/page' must be auto-created");

    let parent = graph.db.get_parent_page("test/page").unwrap();
    assert!(parent.is_some(), "get_parent_page must return Some for test/page");
...
fn test_hierarchy_children_lookup() {
    ...
    assert_eq!(web_titles, vec!["project/web/backend", "project/web/frontend"],
        "unexpected children of project/web: {:?}", web_titles);
    ...
    assert!(proj_titles.contains(&"project/mobile"),
        "project/mobile must appear as child of project; got {:?}", proj_titles);
}
...
fn test_hierarchy_backslash_normalization() {
    ...
    assert_eq!(links[0].to_page_id, target.id, "link must point to test/page (not test\\page)");
}
...
fn test_hierarchy_mixed_slash_normalisation() {
    ...
    graph.db.get_page_by_title_ci("a/b").expect("ancestor 'a/b' must exist");
    graph.db.get_page_by_title_ci("a/b/c").expect("ancestor 'a/b/c' must exist");
    graph.db.get_page_by_title_ci("a/b/c/d").expect("leaf 'a/b/c/d' must exist");
}
...
fn test_hierarchy_auto_create_chain() {
    ...
    for title in &["a", "a/b", "a/b/c", "a/b/c/d"] {
        ...
        assert!(p.is_ok() && !p.unwrap().id.is_empty(),
            "ancestor '{}' must have been auto-created", title);
    }
}
