...
pub(crate) fn apply_writing_changes_to_graph(
    ...
    for change in changes {
        ...
        grafium_core::ai::writing::validate_protected_source(
...
mod tests {
    ...
    fn reading_annotations_and_reference_markers_survive_writing_apply_with_full_snapshot() {
        ...
        let note = graph
            .reading_note_create(
                "03771460-1c34-4544-b535-d6e30c08b141",
                &page.id,
                None,
                "USER-ANNOTATION",
            )
        ...
        let author = before
            .iter()
            .find(|b| b.content.contains("utilize"))
            .unwrap();
        let footer = before
            .iter()
            .find(|b| Some(&b.id) == note.note_block_id.as_ref())
            .unwrap();
        ...
        assert!(apply_writing_changes_to_graph(
            &graph,
            path,
            &page.id,
            &[change(footer, "rewritten annotation".into())],
            Some(&before)
        )
        .is_err());
        assert!(apply_writing_changes_to_graph(
            ...
            &[change(
                author,
                grafium_core::parser::strip_reading_note_references(&author.content)
            )],
            Some(&before)
        )
        .is_err());
        assert!(apply_writing_changes_to_graph(
            ...
            path,
            &page.id,
            &[change(author, author.content.replace("[^1]", ""))],
            Some(&before)
        )
        .is_err());
        ...
        assert!(
            apply_writing_changes_to_graph(&graph, path, &page.id, &safe, Some(&partial)).is_err()
        );
        apply_writing_changes_to_graph(&graph, path, &page.id, &safe, Some(&before)).unwrap();
        assert_eq!(
            graph.db.get_block_by_id(&author.id).unwrap().content,
            safe[0].after_content
        );
        assert_eq!(
            graph.db.get_block_by_id(&footer.id).unwrap().content,
            footer.content
        );
    }
