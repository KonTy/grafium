...
#[tauri::command(rename_all = "camelCase")]
pub fn get_property_values(state: State<AppState>, key: String, source: String) -> Result<Vec<String>, String> {
    let graph = state.graph.lock().map_err(|e| e.to_string())?;
    graph.db.get_property_values(&key, &source).map_err(|e| e.to_string())
}
