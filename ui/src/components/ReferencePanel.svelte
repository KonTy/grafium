<script lang="ts">
  ...
  // References are generated by a background job, so they arrive on an event
  // rather than as a return value. Subscribing for the panel's whole lifetime
  // means a run started before the user navigated away still lands here when
  // they come back to the page.
  onMount(() => {
    let unlisten: (() => void) | undefined;
    let disposed = false;

    onReferencesGenerated((incomingPageId, meta) => {
      if (incomingPageId !== pageId) return;
      references = meta;
    })
      .then((fn) => {
        // The component may already have been destroyed by the time the
        // listener registration resolves; drop it immediately if so.
        if (disposed) fn();
        else unlisten = fn;
      })
      .catch(() => {});
  ...
  async function generateReferences() {
    ...
    isGenerating = true;
    try {
      // Returns as soon as the job is queued. The work continues even if this
      // panel closes, and finishes with a notification either way.
      generateJobId = await aiGenerateReferences(pageId);
    } catch (e: any) {
      isGenerating = false;
  ...
  // Clear the in-progress state from the job's terminal status rather than
  // from the result event alone — otherwise a failed or cancelled run would
  // leave the button stuck on "Analyzing…" forever.
  $effect(() => {
    ...
    if (job.status === "failed") {
      error = job.error ?? "Failed to generate references";
    }
  });
          ...
          <div class="tab-content">
            <div class="tab-actions">
              <button
                ...
                disabled={isGenerating || !pageId}
              >
                {isGenerating ? "Analyzing…" : "Research this page"}
              </button>
              {#if health}
            ...
            {:else if isGenerating}
              <div class="panel-notice">
                Analyzing this page. You can keep working — the result will
                appear here, and you'll be notified when it's ready.
              </div>
            {:else}
              <div class="panel-notice">
